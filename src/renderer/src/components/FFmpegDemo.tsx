...
const FFmpegDemo: React.FC<FFmpegDemoProps> = () => {
  ...
  const [audioBlobUrl, setAudioBlobUrl] = useState<string>(''); // 新增：存储Blob URL
  ...
  const handleExtractThumbnail = async () => {
    ...
    try {
      ...
      const result = await window.ffmpeg.extractThumbnail(
        inputFilePath,
        thumbnailPath,
        '00:00:01'
      );
  ...
  const handleExtractAudio = async () => {
    ...
    try {
      ...
      if (result.success && result.audioPath) {
        ...
        // 通过IPC调用主进程读取文件
        try {
          const audioBuffer = await window.fileSystem.readFile(result.audioPath);
          if (audioBuffer) {
  ...
  return (
    <div className="ffmpeg-demo" style={{ padding: '20px', maxWidth: '600px', margin: '0 auto', overflow: 'auto' }}>
      <h2>FFmpeg 功能演示</h2>
      ...
      <div style={{ marginTop: '30px', fontSize: '14px', color: '#666' }}>
        <h3>功能说明:</h3>
        <ul>
          <li><strong>转换视频:</strong> 将输入视频转换为MP4格式，支持自定义编码、比特率和分辨率</li>
          <li><strong>提取缩略图:</strong> 从视频中提取指定时间点的缩略图</li>
          <li><strong>获取视频信息:</strong> 获取视频的基本信息</li>
        </ul>
      </div>
    </div>
  );
